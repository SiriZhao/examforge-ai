...
ExamForge AI v0.6.0 可以作为无需注册的匿名复习项目服务运行。用户在浏览器获得随机工作空间 ID 与 secret；服务端不配置默认模型 Key。

```bash
docker build -t examforge-ai:0.6.0 .
docker run --rm -p 8000:8000 --env-file .env.example -v examforge_data:/data examforge-ai:0.6.0
```
